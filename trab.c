...
int main() {
    ...
    // Folha N-1 eh a raiz
    folhas[N - 1] = iniciaNo(NULL, 0);

    // Demais folhas
    for (int i = 0; i < N - 1; i++) {
        folhas[i] = iniciaNo(folhas[N - 1], 1);
    }

    // Palavra de teste
    char palavra[N] = "ACTGTAC!";
    int tamPalavra = N - 1;
    ...
    for (int i = tamPalavra - 1; i >= 0; i--) {
        ...
        if (v == vlinha && v == folhas[N - 1] && v->indicador[posicao] == 0) {
            ...
            vlinha->fim[posicao] = tamPalavra - 1;
            ...
            int novaPosicao = converte_letra_posicao(palavra[vlinha->inicio[posicao]]);
            ...
            novoNo->fim[novaPosicao] = tamPalavra - 1;
